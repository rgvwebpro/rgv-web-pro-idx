...
if (!function_exists('_rwp_usamls_widget_search_form')) {
	...
	/*
	 * RGV Web Pro IDX IFRAME SEARCH FORM FUNCTION
	...
	$_rwp_search_string = '<!-- RGV Web Pro IDX Plugin <contact@rgvwebpro.com> -->
	...
	if ( $_POST == true ) {
				...
				foreach ( $_rwp_usamls_search_names_values as $name => $value ) {
					if ( preg_match( '/item/', $name ) && $name == 'item2' && $value == true ) {
						$_rwp_search_string .= '<strong>Priced from:</strong> <input type="text" value="' . $value . '" name="' . $name . '" style="width:30px;" /> , ';
					}
					elseif ( preg_match( '/item/', $name ) && $name == 'item2a' && $value == true ) {
						$_rwp_search_string .= '<strong>Priced to:</strong> <input type="text" value="' . $value . '" name="' . $name . '" style="width:30px;" /> , ';
					}
					elseif ( preg_match( '/item/', $name ) && $name == 'item90' && $value == true ) {
						$_rwp_search_string .= '<strong>Area:</strong> <input type="text" value="' . $value . '" name="' . $name . '" style="width:75px;" /> ';
					}
					elseif ( preg_match( '/item/', $name ) && $value == true ) {
						$_rwp_search_string .= '<input type="text" value="' . $value . '" id="search-result-' . $value . '" name="' . $name . '" style="width:30px;" /> ';
					}
				}
